...
| P6 | Lifecycle in sim (order→monitor→exit/EOD) | ⬜ not built yet (NEXT — last buildable item) |
...
3. **Entry agents → `deterministic_fallback`** (`avg_super_trend=0.00`, `session=""`). **ROOT-CAUSED + GUARDED (2026-06-08)**, fix pending in live code. Two upstream gaps proven against the happy_path replay: **(a)** `enrichers/lib/advanced.py::compute_session_metrics` resolves `session_phase` via `datetime.now()` instead of the bar's own timestamp — backfill at 21:30 stamps every bar of the day "late"; vocabulary `{pre,early,mid,late}` also doesn't match the fallback gate's `{preopen,closing}`. **(b)** `market_data_multitf.st_consensus` (and every other indicator column) is 100% NULL across all timeframes — the multi-TF aggregator never writes indicators in the replay path, so `trend.timeframes[*].st_consensus → "neutral" → avg=0`. Permanent guards: F2 assertions in `sim/run_scenario.py` + `sim/tests/test_fallback_inputs.py`. Marker `sim/.bug3_fixed` deliberately NOT created — bug remains open until live-code fix lands.
4. **VIX null → regime defaults to "enter".** Sandbox gap (no broker) but the *auto-enter on null* logic needs a guard. Scenario E4. **GUARDED (2026-06-08)** — see §7 item 4. Live fix still pending in `brahmand/unicorn_debate._deterministic_fallback`.
...
5. **Lifecycle G1–G3 + ledger D1** — extend sim past entry into order(paper)/monitor/exit/EOD with `--llm stub`. **NOW THE LAST BUILDABLE ITEM** (milestone "Lifecycle in sim (order→monitor→exit)" — checks for `position_manager` in `run_scenario.py`). This is what the resumed autobuilder will pick up next.
