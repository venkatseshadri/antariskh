"""ORBITER v3.0 specs, multi-day-adapted for PROTON's weekly NIFTY iron condor.

Ports the same 5 module specs retrofitted into ATOM (Tier 1, intraday) —
Multi-Gated Entry, ATR Trailing Stop, Dynamic Take-Profit, Dynamic Legging
(see atom/src/atom/orbiter.py) — into a Friday-entry/Tuesday-expiry hold
instead of a same-day session. ATOM's "DTE Selection" doc has no analog
here: it's weekday->index-alternation for ATOM's own intraday NIFTY/SENSEX
swap, and PROTON already has its own fixed weekly cadence.

Data source: `market_data_enriched` / `market_data_multitf` in
capture_nifty.sqlite — the SAME physical DB ATOM's Penguin pipeline writes
to (see config/sqlite_schema.py). Read-only, mode=ro, same pattern as
monthly_ic_pilot.py's _live_daily_closes(). This couples gate/TP data
freshness to ATOM's enricher uptime — by design, gates fail CLOSED (skip
entry, retry next Friday) when the row is missing, while TSL/TP-array
enhancements fail OPEN (the existing static PT/SL backstop in
weekly_ic_pilot_orbiter.py keeps running regardless, so a stale enricher
never leaves a position unmonitored).

No physical ORBITER doc01-05 file exists anywhere in the repo — "spec" is
atom/src/atom/orbiter.py's actual implementation. Legging Phase 3
(asymmetric breakage) mirrors that file's own status: built, never proven
to change exit behavior even at the source (independent per-leg exit
checks already produce the same outcome) — ported here for parity, not
...
STRIKE_GAP = 50  # NIFTY only, matches monthly_ic_pilot.STRIKE_GAP

ORBITER_CFG = {
    "gate1.adx.max": 25,
    "gate1.cpr_wide_pct_min": 0.25,  # reverted 07-18 recalibration attempt — narrow CPR correctly flagged 2/2 real trend days (07-16/17), so 0 organic passes was the filter working. Real fix needs a redesign (CPR gates 9:15 alone; ADX is intraday-only per doc), not a threshold tweak.
...
def _read_enriched_row(instrument: str = "NIFTY", today: date | None = None) -> dict | None:
    ...
    Fallback (2026-07-17): if today's enriched data is not yet available
    (common early Monday before 09:15 enricher produces its first bar),
    falls back to yesterday's latest enriched row with `_stale_fallback=True`
    in the returned dict. Callers can check this flag to decide whether to
    use the stale data for entry gating or skip."""
...
def gate1_regime(row: dict) -> GateResult:
    ...
    atom/src/atom/orbiter.py (centralized 2026-07-19; previously hand-ported
    3x across atom/orbiter.py, this file, and orbiter_monthly.py — see
    _gate1_regime_core's docstring there for the full history, including
    the structural pre-open window / ADX sanity ceiling / prior-day-ADX
    rejection all documented once, centrally, instead of 3x here)."""
...
def gate1_tiger_override(row: dict, g1: GateResult, bar_ts: str | None = None) -> GateResult:
    ...
    atom/src/atom/orbiter.py (centralized 2026-07-19). `bar_ts` is an ISO
    timestamp string; if None, time check defaults to False (caller is
    responsible for providing the right bar time)."""
...
def gate2_strikes(row: dict, spot: float, wing_strikes: int = 3) -> StrikeMap:
    """Gate 2: anchor condor boundaries off real BB (multitf daily) if present,
    else synthesize from bb_width (same fallback ATOM uses), plus VWAP/MaxPain."""
    ...
    def _nearest(target: float) -> int:
        return round(target / STRIKE_GAP) * STRIKE_GAP

    if bb_upper and bb_lower:
        call_short = _nearest(bb_upper) + STRIKE_GAP
        put_short = _nearest(bb_lower) - STRIKE_GAP
    else:
        call_short = atm + wing_strikes * STRIKE_GAP
        put_short = atm - wing_strikes * STRIKE_GAP

    call_short = max(call_short, atm + wing_strikes * STRIKE_GAP)
    put_short = min(put_short, atm - wing_strikes * STRIKE_GAP)
    call_hedge = call_short + wing_strikes * STRIKE_GAP
    put_hedge = put_short - wing_strikes * STRIKE_GAP

    if max_pain is not None and max_pain != atm:
        if max_pain > atm:
            call_short = max(call_short, max_pain + wing_strikes * STRIKE_GAP)
            call_hedge = call_short + wing_strikes * STRIKE_GAP
        else:
...
def orbiter_tp_check(
    ...
    """Same 5-point priority order as ATOM, EOD_HARD dropped (PROTON's existing
    `today >= expiry` EXPIRY check is the multi-day equivalent — caller runs
    this before that check each tick). `elapsed`/`total_duration` are the
    Friday-entry-to-Tuesday-expiry window, not same-day HH:MM."""
